...
设备连接 MQTT Broker 需携带用户名密码（EMQX 内置数据库认证）：

- **username** = 设备序列号（`serial_number`）
- **password** = 管理端分配的设备凭证：管理页添加设备时随机生成，设备详情页（概览页签）可查看、可复制
- **服务器/端口** = 凭证弹窗同时下发（`mqtt_host` / `mqtt_port`，为设备对外可达地址，非平台内部地址）

规则：

- 重新生成凭证后旧密码**立即失效**，使用旧凭证的设备会断开连接，需将新凭证同步到设备端
- 认证开启后匿名连接与错误密码均被拒绝（`Connection Refused: not authorised`）
- 未开启认证的环境 Broker 匿名放行，设备可不带凭证连接；是否要求凭证以环境部署为准（认证由环境运维在 Broker 侧启用）
